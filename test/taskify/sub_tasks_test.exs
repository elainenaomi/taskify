defmodule Taskify.SubTasksTest do
  ...
  describe "sub_tasks" do
    ...
    test "list_user_sub_tasks/2 returns all sub_tasks" do
      ...
      assert SubTasks.list_user_sub_tasks() == [sub_task]
    end

    test "get_user_sub_task!/3 returns the sub_task with given id" do
      user = Taskify.AccountsFixtures.user_fixture()
      task = Taskify.TasksFixtures.task_fixture(user)
      sub_task = sub_task_fixture(task, user)
      assert SubTasks.get_user_sub_task!(sub_task.id, user, task.id) == sub_task
    end

    test "create_sub_task/3 with valid data creates a sub_task" do
      user = Taskify.AccountsFixtures.user_fixture()
      task = Taskify.TasksFixtures.task_fixture(user)
      valid_attrs = %{completed: true, description: "some description", name: "some name"}

      assert {:ok, %SubTask{} = sub_task} = SubTasks.create_sub_task(valid_attrs, user, task.id)
    ...
    test "update_sub_task/2 with invalid data returns error changeset" do
      ...
      assert sub_task == SubTasks.get_user_sub_task!(sub_task.id, user, task.id)
    end
    ...
    test "delete_sub_task/1 deletes the sub_task" do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        SubTasks.get_user_sub_task!(sub_task.id, user, task.id)
      end
    end
