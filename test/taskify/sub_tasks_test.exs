defmodule Taskify.SubTasksTest do
  ...
  describe "sub_tasks" do
    ...
    test "get_user_sub_task!/3 returns the sub_task with given id" do
      sub_task = sub_task_fixture()
      assert SubTasks.get_user_sub_task!(sub_task.id) == sub_task
    end
    ...
    test "update_sub_task/2 with invalid data returns error changeset" do
      ...
      assert sub_task == SubTasks.get_user_sub_task!(sub_task.id)
    end

    test "delete_sub_task/1 deletes the sub_task" do
      sub_task = sub_task_fixture()
      assert {:ok, %SubTask{}} = SubTasks.delete_sub_task(sub_task)
      assert_raise Ecto.NoResultsError, fn -> SubTasks.get_user_sub_task!(sub_task.id) end
    end
